...
В отличие от наиболее близкого аналога «RunGo», «SightWalk» должно создавать пешеходные маршруты с учётом выбранной категории достопримечательностей, а также предоставлять статистику не только по конкретной тренировке, но и по количеству километров, пройденных другими участниками. 
Если сравнивать с приложениями «Шаги ВКонтакте» и «Открывая Москву», то разрабатываемое приложение должно совмещать функционал обоих приложений.
...
Продукт должен являться мобильным приложением для Android, IOS и иметь user-friendly интерфейс. Приложение должно быть реализовано с помощью React Native и написано на языке JavaScript. Карты местности будут храниться в файлах с расширением KML, названия которых соответствуют названию регионов, а статистические данные пользователей - в базе данных MySQL.
